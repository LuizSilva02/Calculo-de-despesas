...
const addTransactionIntoDom = transaction => {
    ...
    const amounWithoutOperator = Math.abs(transaction.amount)
    const li = document.createElement('li')

    li.classList.add(CSSClass)
    li.innerHTML = `
     ${transaction.name} 
     <span>${operator} R$ ${amounWithoutOperator}</span>
...
const updadteBalanceValues = () => {
    ...
    .filter(value => value > 0).reduce((acumulator,value) => acumulator + value, 0)
    .toFixed(2)
